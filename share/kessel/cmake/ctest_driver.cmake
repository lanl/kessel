set(CTEST_SOURCE_DIRECTORY "$ENV{KESSEL_PIPELINE_SOURCE_DIR}")
set(CTEST_BINARY_DIRECTORY "$ENV{KESSEL_PIPELINE_BUILD_DIR}")
set(CTEST_PROJECT_NAME "$ENV{KESSEL_ENV_PROJECT}")


set(CTEST_SITE "$ENV{KESSEL_SYSTEM}")
if("$ENV{CI_PIPELINE_SOURCE}" STREQUAL "merge_request_event")
  set(CTEST_BUILD_NAME "$ENV{KESSEL_ENVIRONMENT}, source=$ENV{CI_MERGE_REQUEST_SOURCE_BRANCH_NAME}, target=$ENV{CI_MERGE_REQUEST_TARGET_BRANCH_NAME}, MR=$ENV{CI_MERGE_REQUEST_IID}")
else()
  set(CTEST_BUILD_NAME "$ENV{KESSEL_ENVIRONMENT}, Branch=$ENV{CI_COMMIT_REF_NAME}")
endif()
...
if(${CTEST_SCRIPT_ARG} MATCHES Configure)
  ...
  if(configure_error)
    if(${CTEST_SCRIPT_ARG} MATCHES ReportErrors)
        ctest_submit(HTTPHEADER "Authorization: Bearer $ENV{CTEST_TOKEN}")
    endif()
